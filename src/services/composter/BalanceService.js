...
const getList= async()=>{ 
  try{
    const list = await Balance.findAll({ attributes: { exclude: ['createdAt', 'updatedAt'] }});
...
const insert= async(body)=>{
  try{
    var res = await Balance.create(body)
    return { message: 'saldo de produccion ingresado', status:200 };
  }catch(err){
